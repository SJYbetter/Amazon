...
public class GameOfLife2 {
    ...
    private void InternelGameOfLife2(int nColumn) throws  IOException {
        int[] dx = {1, -1, 0, 0, 1, -1, 1, -1};
        int[] dy = {1, -1, 1, -1, 0, 0, -1, 1};
        int i=0;
        while (readBatch(nColumn, 3)) {
            for (; i < nRow; i++) { // lineNo 每次 readBatch会增加
                int dead = 0, live = 0;
                for (int j = 0; j < nColumn; j++) {
                    for (int k = 0; k < 8; k++) {
                        int x = i + dx[k];
    ...
    public void GameOfLife2(String in, String out, int nBytes) throws IOException {
...
//                    int x = i + dx[k];
//                    int y = j + dy[k];
//                    System.out.println(String.format("(%d, %d) -> (%d, %d)", i,j, x,y));
//                }
//            }
//        }
    }
...
//    private void read_line(BufferedInputStream input, byte[] bytes){
//
//    }
